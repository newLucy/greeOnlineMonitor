...
export default {
  ...
  mounted () {
    this.tableArray.tableHeader = window.$vueI18n.t('m.manage.logs.tableHeader')
    this.handleSearch()
  },
  computed: {
    org () {
  ...
  methods: {
    ...
    getTableData () {
      ...
      let params = {
        account: this.form.condition.trim(),
        orgId: this.org, // this.org
    ...
    handleDownload () {
      location.href = `${this.$url}/log/exportLog?account=${this.form.condition.trim()}&orgId=${this.org}`
    },
